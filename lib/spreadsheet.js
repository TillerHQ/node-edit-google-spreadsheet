...
function HTTPResponseError(response, body) {
  this.name = 'HTTPResponseError';
  this.message = "HTTP Response error " + response.statusCode;
  if (body) {
    this.message += ". See body property for additional detail.";
  }
...
Spreadsheet.prototype.receive = function(options, qs, callback) {
  if (typeof options === 'function') {
    callback = options;
    options = {};
  ...
  this.request({
    ...
    _this.log('spreadsheet.receive err:'.green, err);
    if (!result || !result.feed) {
      err = new Error("Error Reading Spreadsheet");
      _this.log(
        err.message.red.underline +
    ...
    _this.log('spreadsheet.receive entries:'.green, entries.length);
    ...
    _this.log('spreadsheet.receive transforming cells'.green);
